...
{
	console.log("%cERROR"+"%c "+msg, 'background:#ff2400; border-radius: 2px; font-size: 10pt; padding: 2px;', 'background:default');
...
chrome.runtime.onMessage.addListener((req, sender, sendRes) => {
	...
	switch(req.action)
	{
		...
		case "bages":
			...
			storage.get(["stages", "mode", "profilearr"], res => {
				arr.push('Chat');
				console.log(res.stages.concat(arr));
				storage.set({stages: res.stages.concat(arr), stage: 1, profilearr: res.profilearr.concat(profilearr)}, queue);
			});
			break;
		case "start":
			storage.get(['custom', 'customsettings'], (res) => {
				if(!window.working)
					if(!req.continue)
					{
						...
						let profilearr = [];
						let needProfileEdit = false;

						if(req.mode == 5)
						{
							if(res.customsettings.nameon)
							{
								profilearr.push('SetupCommunityRealName');
								needProfileEdit = true;
							}

							if(res.customsettings.infoon) {
								profilearr.push('AddSummary');
								needProfileEdit = true;
							}
							if(needProfileEdit)
								stages.push('ProfileEdit');
							...
							if(res.customsettings.groupon)
								stages.push('JoinGroup');

							storage.set({stages: stages, profilearr: profilearr});
							Init();
							return;
						}
